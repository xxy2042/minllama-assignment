...
class LlamaDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        encoding = [
            self.tokenizer.encode(s, bos=True, eos=self.eos) for s in sents
        ]
        max_length_in_batch = max([len(sentence) for sentence in encoding])
        encoding_padded = [
            sentence
            + [self.tokenizer.pad_id] * (max_length_in_batch - len(sentence))
...
def train(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for step, batch in enumerate(
            ...
            logits = model(b_ids)
            loss = (
                F.nll_loss(logits, b_labels.view(-1), reduction="sum")
                / args.batch_size
            )
...
def generate_sentence(
    args, prefix, outfile, max_new_tokens=75, temperature=0.0
):
    with torch.no_grad():
        device = get_device(args.use_gpu)
        ctx = (
            torch.amp.autocast(device_type=device.type, dtype=torch.float32)
            if args.use_gpu
            else nullcontext()
        )
...
def test_with_prompting(args):
    ...
    with torch.no_grad():
        ...
        if len(label_names) == 2:
            ...
            label_name_str = (
                ", ".join(label_names[:-1]) + ", or " + label_names[-1]
            )
...
def test(args):
    assert args.dev_out.endswith("dev-finetuning-output.txt"), (
        'For saving finetuning results, please set the dev_out argument as "<dataset>-dev-finetuning-output.txt"'
    )
    assert args.test_out.endswith("test-finetuning-output.txt"), (
...
def get_args():
    ...
    parser.add_argument(
        "--pretrained-model-path", type=str, default="stories42M.pt"
    )
